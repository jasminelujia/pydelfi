...
def log_likelihood(theta, d, sim_args, prior_args):
    ...
    if truncated_gaussian_prior(theta, prior_args) == 0:
        ...
        lnL = lnL + np.log(truncated_gaussian_prior(theta, prior_args))
...
lower = np.zeros(npar)
upper = np.zeros(npar)
lower[0:6] = np.array([0, 0.4, 0, 0.4, 0.7, -1.5])
lower[6:] = np.ones(nz)*-0.1
upper[0:6] = np.array([1, 1.2, 0.1, 1.0, 1.3, 1.5])
upper[6:] = np.ones(nz)*0.1
prior_mean = theta_fiducial
sigmas = np.zeros(npar)
sigmas[0:6] = np.array([0.1, 0.1, 0.00015, 0.005, 0.3, 0.3])
sigmas[6:] = np.ones(nz)*0.05
...
nwalkers = 50
nsamples = 200
Finv = np.loadtxt(open('Finv_full_de.dat', 'rb'))
x0 = [truncated_gaussian_prior_draw([prior_mean, Finv, lower, upper]) for i in range(nwalkers)]
D = 16
...
# Burn-in chain
pos, prob, state = sampler.run_mcmc(x0, nwalkers)

# Save the chain
f = open('emcee_samples_systematics_de.dat', 'wb')
...
for i in range(10):
    ...
    # Save the chain
    f = open('emcee_samples_systematics_de{}.dat'.format(i+1), 'wb')
    np.savetxt(f, sampler.flatchain)
    f.close()
