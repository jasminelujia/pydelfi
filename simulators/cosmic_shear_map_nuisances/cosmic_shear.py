...
def cl_to_cl_hat(cls, l_min, l_max, sig_n_p, mask, seed=None):
    ...
    # project to map and add pixel noise and mask
    ...
    for i in range(n_fields):
        for j in range(i + 1):
            c_l_hat[i, j, :] = hp.anafast(field[i, :], field[j, :], \
                                          lmax=l_max, mmax=l_max, \
                                          iter=3)[l_min:]
...
def projected_score(d, projection_args):
    ...
    # Make it an MLE
    t = np.dot(Finv, dLdt) + theta_fiducial + np.dot(Finv, np.dot(Qinv, prior_mean - theta_fiducial))
